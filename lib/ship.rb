...
require_relative 'weapons/base_shot'
require_relative 'weapons/double_shot'
require_relative 'weapons/nuke'
require_relative 'weapons/single_shot'
require_relative 'weapons/super_shot'
...
class Ship
  ...
  def self.load_explosion_sound window
    @explosion_sound ||= Gosu::Sample.new(window, "media/explosion.ogg")
  end
  ...
  def reset_shield_counter
    @shield_counter = -30 unless @cheat_energy
  end
  ...
  def toggle_cheat_energy
    ...
    @shield_counter =MAX_SHIELD_ENERGY if @cheat_energy
  end
  ...
  def collide? obj
    if shield_up?
      if @shield.collide?(obj)
        return true
      end
    else
      if can_collide? && GameUtilities.collide?(self, obj)
        reset_score
        # collision with ship destroys ship
        self.destroy
        return true
      end
    end

    # check if any of the shots collide with the object
  ...
  def fire_nuke
    if @shield_counter >= MAX_SHIELD_ENERGY
      ...
      reset_shield_counter unless @cheat_energy
    end
  end
end
